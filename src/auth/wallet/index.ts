...
export const walletAuth = async (
  ...
  async function hasNode(address: string): Promise<boolean> {
    const payload = { address: address };
    const res = await fetch("/api/user-node", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    if (!res.ok) return false;
    const data: { node: boolean } = await res.json();
    return !!data.node;
  }
  const exists = await hasNode(result.finalPayload.address);
